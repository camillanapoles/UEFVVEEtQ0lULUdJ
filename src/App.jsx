      ...
      <div className="relative w-full max-w-2xl bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 sm:p-8 pr-16 sm:pr-20">
          ...
          <div className="space-y-4 mb-6">
            {question.contract && (
              <Section icon={<FileText size={14} />} label="Contrato" text={question.contract} />
            )}
...
function Section({ icon, label, text, highlight }) {
  const cls = highlight ? "bg-slate-900 text-white" : "bg-slate-50";
  const labelCls = highlight ? "text-slate-300" : "text-slate-500";
  const textCls = highlight ? "text-white" : "text-slate-800";
  return (
